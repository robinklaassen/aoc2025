...
fn parse_line(line: String) -> i32 {
    ...
    let count = chars.as_str().parse::<i32>().expect("should parse string into int");
...
fn count_zero_passes(start: i32, rotation: i32) -> i32 {
    ...
    if new_position >= 100 {zero_passes += 1};  // zero pass while turning right
    if start != 0 && new_position <= 0 {zero_passes += 1}  // zero pass while turning left
...
pub fn main() {
    ...
    println!("Part 1 answer: {}", part1(&input_lines));
    ...
    println!("Part 2 answer: {}", part2(&input_lines));
}